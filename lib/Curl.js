...
(function() {
  ...
  Curl = require("bindings")("node_curl.node").Curl;
  ...
  Curl.prototype.setopt = function(option_name, value) {
    ...
    if ((option_id = Curl.user_options[option]) != null) {
      if (option === 'MULTIPART') {
        ret = this.setopt_httppost(value);
      } else {
        ret = this.setopt_user_(option_id, value);
      }
    } else if ((option_id = Curl.slist_options[option]) != null) {
      ret = this.setopt_slist_(option_id, value);
    } else if ((option_id = Curl.integer_options[option]) != null) {
      ret = this.setopt_int_(option_id, value >> 0);
    } else if ((option_id = Curl.string_options[option]) != null) {
      if (value == null) {
        throw new Error("Cannot set option " + option_name + " to " + value + ".");
      }
      ret = this.setopt_str_(option_id, value.toString());
    } else {
      throw new Error("unsupported option " + option);
    }

    /* Check return code from curl_easy_setopt */
    if(ret){
      throw new Error("curl_easy_setopt error " + ret);
    }
  ...
  Curl.prototype.on = function(event, callback) {
    ...
    switch (event) {
      ...
      case 'header':
        ...
        break;
      case 'progress':
        this.on_progress = function(dlt, dln, ult, uln) {
          return callback.call(_this, dlt, dln, ult, uln);
        };
        break;
      case 'error':
